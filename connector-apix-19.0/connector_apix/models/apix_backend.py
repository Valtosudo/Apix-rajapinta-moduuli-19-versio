...
class ApixBackend(models.Model):
    ...
    _company_uniq = models.Constraint(
        "unique(company_id)", "Company can have only one backend."
    )
    ...
    def action_einvoice_fetch(self):
        for record in self:
            record.with_company(record.company_id.id).list_invoices(refetch=False)

    def action_einvoice_refetch(self):
        for record in self:
            record.with_company(record.company_id.id).list_invoices(refetch=True)
    ...
    def list_invoices(self, refetch=False):
        ...
        for invoice in invoices.findall(".//Group"):
            ...
            if (
                ...
                self.with_company(self.company_id.id).download_invoice(
